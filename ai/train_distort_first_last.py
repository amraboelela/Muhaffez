...
def distort_word(word):
    """Distort a word by removing 1-2 random characters"""
    if len(word) <= 2:
        return word

    chars = list(word)
    num_to_remove = random.randint(1, min(2, len(chars) - 1))
...
class DistortFirstLastWordDataset(Dataset):
    """Dataset that distorts first and last words in the 60-char input"""
    ...
    def __getitem__(self, idx):
        ...
        # 70% chance to distort first and/or last word
        if random.random() < 0.7:
            words = ayah_text.split(' ')

            if len(words) > 1:
                # Distort first word (50% chance)
                if random.random() < 0.5:
                    words[0] = distort_word(words[0])

                # Distort last word (50% chance)
                if random.random() < 0.5:
                    words[-1] = distort_word(words[-1])
...
def main():
    ...
    # Create dataset that distorts first and last words
    dataset = DistortFirstLastWordDataset(ayat, vocabulary, max_length=60)
    ...
    # Train model
    print('\nTraining with DISTORT FIRST AND LAST WORD (70% probability, remove 1-2 chars)...\n')
