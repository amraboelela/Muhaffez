...
class OmitFirstWordQuranDataset(Dataset):
    """Dataset that omits the first word - 50% from beginning, 50% omitting first word"""
    ...
    def __getitem__(self, idx):
        ...
        # 50% chance to omit first word
        if random.random() < 0.5:
            # Find first space and skip first word
            space_idx = ayah.find(' ')
            if space_idx > 0 and space_idx < len(ayah) - 1:
                # Skip first word (start after the space)
                ayah_text = ayah[space_idx + 1:space_idx + 1 + self.max_length]
            else:
                # No space found or space at end, use from beginning
...
def main():
    ...
    # Create dataset that omits first word 50% of the time
    dataset = OmitFirstWordQuranDataset(ayat, vocabulary, max_length=60)
    ...
    # Train model
    print('\nTraining with OMIT FIRST WORD augmentation (50% from start, 50% skip first word)...\n')
